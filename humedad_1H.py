...
DF=pd.DataFrame(data, columns=['fecha','humedad'])
...
DF3 = pd.DataFrame(data2, columns=['humedad'])
DF3['humedad'] = DF3['humedad'].astype(float)
...
while (aux < int(DF3.count())):
    while (valor >= 0):
        if ((aux - valor)< 0):
            ...
            #print (DF3['humedad'][aux-valor])
            Array.append(DF3['humedad'][aux-valor])
            valor = valor - 1
    valor = 4
    aux = aux + 1
